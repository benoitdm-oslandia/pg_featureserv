...
func insertSimpleDataset(db *pgxpool.Pool) {
	...
	// Separate requests to drop trigger, because CREATE OR REPLACE TRIGGER only since postgresql 14
	dropTriggerBytes := []byte(`
	DROP TRIGGER IF EXISTS %[1]s_notify_event ON %[1]s;
	`)
	triggerBytes := []byte(`
	...
	for tableName, tableElements := range tablesAndExtents {
		...
		dropTriggerStatement := fmt.Sprintf(string(dropTriggerBytes), tableName)
		triggerStatement := fmt.Sprintf(string(triggerBytes), tableName)
		b.Queue(dropTriggerStatement)
...
func insertComplexDataset(db *pgxpool.Pool) {
	...
	dropTriggerStatement := `
		DROP TRIGGER IF EXISTS mock_multi_notify_event ON mock_multi;
		`
	triggerStatement := `
		CREATE TRIGGER mock_multi_notify_event
